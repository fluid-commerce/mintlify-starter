...
<Steps>
  ...
  <Step title="Add Items to the Cart">
    ...
    **Option C — Direct API call**
    ...
    Use `POST /api/checkout/v2026-04/carts/{cart_token}/items` to add line items server-side. The body takes an `items` array — each entry needs a `variant_id` and a `quantity`:
