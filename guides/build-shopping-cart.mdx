...
<Steps>
  ...
  <Step title="Fetch Product Details">
    ...
    The product is returned under a `product` envelope. It carries the display `title`, `pricing`, stock status, media, and a `variants` array — each variant has the `id` you pass when adding items to the cart. See the [Storefront endpoints](/api-reference/storefront/public-product-by-slug) reference for the full response schema.
  </Step>
  ...
  <Step title="Add Items to the Cart">
    ...
    **Option C — Direct API call**
    ...
    See the [Checkout endpoints](/api-reference/cart-items/add-items-to-cart) reference for the full request and response schema.
  </Step>
