...
<Steps>
  ...
  <Step title="Listen for cart events and validate every one">
    ...
    handshake or an iframe reload — carries the same number as the message it
    repeats, so the guard above makes it a no-op.
  </Step>

  <Step title="Read the cart and render">
    Read the cart from **your own server** with [Show a cart](/api-reference/carts/show-a-cart).
    The cart token in the path is the credential — no bearer token is needed,
    and none should ever ship inside an embed. Your page asks your server, and
    your server asks Fluid:
    ...
    async function refreshFromCart() {
      ...
      inFlight = (async () => {
        // Your server calls GET /api/checkout/v2026-04/carts/{cart_token}.
        const response = await fetch(`/widget/cart?token=${encodeURIComponent(cartToken)}`);
        if (response.status === 404) return; // not a cart token on this surface
    ...
    one. Rendering from the cart you fetched, never from the message, means a
  ...
  <Step title="Keep a fallback for surfaces without events">
    ...
    — not a cart token — so do not read the cart there at all. Keep a slow poll
    — 30 seconds or more, paused while the tab is hidden — stop it the first
    time a valid event arrives, and stop it if a read answers `404`, which means
