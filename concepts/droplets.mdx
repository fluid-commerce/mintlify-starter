...
description: "Droplets are modular integrations for the Fluid platform. Install pre-built apps from the Droplet Marketplace or build your own."
---
...
<Steps>
  ...
  <Step title="Verify the webhook signature">
    Validate the request signature using your Droplet's signing secret to confirm the request genuinely came from Fluid.
  </Step>
  <Step title="Exchange the token">
    Make a `POST` request to the Fluid API with the exchange token to receive a long-lived access token and refresh token for the installing company.
  </Step>
  <Step title="Store the credentials">
    Persist the long-lived credentials securely. Use them to make authenticated API calls on behalf of the company for the lifetime of the installation.
  </Step>
</Steps>
