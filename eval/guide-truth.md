...
| `auth` | `path`, `method`, `auth: "none"\|"bearer"`, `scopes?: [string]` | `none` → op has no security requirement (or empty); `bearer` → op requires a bearer/http scheme. `scopes` (optional, non-empty string array, valid **only** with `auth: "bearer"`) asserts the op grants each named scope: the spec models scopes as an OAS 3.1 role-name list on the bearer requirement entry (e.g. `security: [{ bearer_auth: ["storefront.update"] }]`, scheme stays http/bearer), and each scope in `scopes` must be a member of that list — a missing scope (including an empty `[]` grant) **fails mechanically** |
