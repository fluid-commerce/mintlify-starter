...
<Note>
  The company index accepts neither `filter[parent_id]` nor `filter[source_type]` — those two filters exist only on the public catalog. To group rows by parent as an admin, page through the index and group client-side by each row's `parent_id`. See [Work with the category hierarchy](/api/guides/category-hierarchy).
</Note>
