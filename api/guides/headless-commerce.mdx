...
<Steps>
  <Step title="Find a product and a variant">
    ...
    The response lists the product's purchasable variants; see the [Checkout endpoints](/api-reference/products/query-product) reference for its full shape. To browse or search the whole catalog instead of fetching one product, use the Storefront products API, documented under the Storefront endpoints reference.
  </Step>
...
- Browse the [Checkout endpoints](/api-reference/carts/create-a-cart) reference for full request and response schemas.
