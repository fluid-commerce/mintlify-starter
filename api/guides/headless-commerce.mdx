...
Cart mutations authenticate with a company Bearer token, sent as `Authorization: Bearer <your_token>` on every request. Three operations are public and need no token: creating a cart, querying a product, and retrieving a finished order. A server-side integration typically holds one company token and uses it for the whole flow.

<Info>
  The cart token appears as `{token}` in cart-level paths and as `{cart_token}` in the items and discount sub-resources. It is the same value in both — the `cart_token` the create-cart response returns. Substitute it wherever a path shows either placeholder.
</Info>
...
<Steps>
  ...
  <Step title="Add and adjust line items">
    Add more items after creation with `POST /api/checkout/v2026-04/carts/{cart_token}/items`. The body takes an `items` array; each entry requires a `variant_id` and a `quantity`. Mark an item as a subscription by setting `subscription` to true and naming a `subscription_plan_id`.
    ...
    You can inspect the cart at any time with `GET /api/checkout/v2026-04/carts/{token}`, which returns the current items, totals, and available shipping methods.
  ...
  <Step title="Capture the buyer's contact details">
    Attach the buyer's contact information with `PATCH /api/checkout/v2026-04/carts/{token}`. The body accepts `email`, `phone`, and the marketing opt-ins `email_marketing` and `sms_marketing`.
  ...
  <Step title="Set the shipping address">
    Set the address with `POST /api/checkout/v2026-04/carts/{token}/address`. The body requires a `type` — one of `ship_to`, `bill_to`, or `both` — and carries the address fields under an `address` object.
  ...
  <Step title="Select a shipping method">
    Once the cart has an address, choose one of its available methods with `PUT /api/checkout/v2026-04/carts/{token}/shipping`, passing the `shipping_method_id`. Selecting a shipping method is idempotent — selecting the same one again is a no-op.
  ...
  <Step title="Complete the checkout">
    Turn the cart into an order with `POST /api/checkout/v2026-04/carts/{token}/complete`. Pass the payment reference as the `payment_uuid` query parameter, and send an `Idempotency-Key` header so a retried request never places a second order. A successful call returns `200` with the completed order.
