...
The public catalog and public slug endpoints also return `countries`, but only for live rows.
...
<Warning>
  For categories and collections, country availability is controlled and queried with `filter[country]`. Do **not** use the `country` query parameter for this — `country` is a product-only parameter that selects country-relative pricing, and it does not filter category or collection availability.
</Warning>
