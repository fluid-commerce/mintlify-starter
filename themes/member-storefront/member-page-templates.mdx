...
description: "Create the member layout, a home page and custom member pages, and read the member, member_routes, and settings.menu values they receive."
---
...
<Note>
  No root theme ships member page templates or a member layout. Until a site has a published page with the `/` slug, the account subdomain root returns a not-found response for its members.
</Note>
...
| --- | --- | --- |
| `members_page` | `/{slug}`, or `/` for the home page | One template per member page you add to a site. The page with the `/` slug is the site's home page |
...
Member page templates are created with their pages in the theme editor, including the home page. Authentication uses Fluid’s hosted sign-in screen.
...
Add a page with the `/` slug to the site. It is served at the site root, so it is the first page a member sees after signing in. Compose its `members_page` template, for example `members_page/home/index.liquid`:
...
<section class="member-home">
  ...
  <div class="member-home__links">
    {% for item in settings.menu.menu_items %}
      <a class="member-home__card" href="{{ item.url }}">{{ item.title }}</a>
    {% endfor %}
  </div>
</section>
...
The home page renders inside whatever layout the member's site names. Do not add a `{% layout %}` tag to it or to any other member template. The site decides the frame, and a tag in the template is ignored.
...
Every member page, including the home page, receives the same values: `member`, `member_content`, `member_routes`, and `settings.menu`. It renders inside its site's layout. Its slug and its site decide where it is served and who can open it, both of which are set on the page rather than in the template. See [Add pages to a site](/themes/member-storefront/member-sites#add-pages-to-a-site).
...
- A member who matches no site, an unpublished page, and a page in another member's site all return a not-found response. A site without a published `/` page returns a not-found response at the root.
...
1. Sign in as a member and open the home page. Confirm the layout, greeting, and menu render, and that no storefront header appears.
...
| --- | --- |
| The home page returns not found | Confirm the member's site has a published page with the `/` slug, the member matches an active site, and member storefront is enabled. |
...
- Add a published page with the `/` slug to each site and compose its `members_page` template as the home page. Use `member_routes.sign_in` for sign-in links; authentication does not render a theme page.
...
- Verify the home page, a custom page, a system screen, sign-out, and the guest sign-in flow.
...
Add a published home page with the / slug to each site, and give its members_page template
a greeting from member.first_name and a section slot.
...
Verify the home page, a custom page, a system screen, sign-out, and the guest sign-in redirect.
