...
| Member page | A page you add to one site. It has its own template and its own slug. The page with the `/` slug is the site's home page |
...
A default site therefore never shadows an explicit one, wherever it sits in the list. An inactive site resolves for nobody. A member who matches nothing is refused: every member page, including the home page and the system screens, returns a not-found response for them.
...
- A page has a title, a slug, and a published flag. The slug is derived from the title unless you set one. It uses lowercase letters, numbers, and hyphens, and is at most 64 characters. Set the slug to `/` to make the page the site's home page, served at the site root.
...
The system screens are not pages you add. Every site has all of them.
...
Until a site has a menu with items, Fluid derives one from the site's published pages by title, including the home page. A menu whose items all resolve to nothing falls back the same way, so a member always has a way out of the page they landed on.
...
Use `member_routes.sign_in` and `member_routes.sign_out` for ordinary navigation links. Both the storefront and account host use hosted authentication; no themed sign-in page is rendered. See [Sign in and sign out](/themes/member-storefront/authentication) for examples and release requirements.
...
Exporting a theme writes a version 2 `members_sites.json` at the theme root. It contains sites, portable access rules, custom pages, complete menus, template metadata, and references to menu settings. Template content remains in separate files.

Importing or uploading the document reconciles the receiving theme's member configuration. Sites match by name and pages by slug within their site. Missing sites and pages are removed from that theme, so preserve unrelated entries when editing a single menu. Country codes and member-type slugs resolve against the receiving company; missing dependencies or unresolved rules reject the member configuration instead of silently widening access.

You can edit the manifest while `fluid theme dev` is running. The CLI uploads the change and reloads its preview after success. See the [Member sites manifest](/themes/member-storefront/manifest) for the complete format, navigation examples, and removal behavior.

## Test a member site

1. Sign in as a member who matches the site's rule. Confirm the home page, each published page, and each system screen open.
...
| --- | --- |
| Every member page returns not found | Confirm member storefront is enabled for the company, the member's site has a published page with the `/` slug, at least one active site matches the member, and you are on the account subdomain. |
...
| Imported sites match nobody | Check the imported country codes and member-type slugs against the member viewing the site. Missing destination values reject a version 2 import. |
...
- Use the theme editor or edit the complete `members_sites.json` through the theme CLI. Verify the target theme before uploading, and preserve unrelated entries.
...
page to the Reps site and link it, the home page, and the Contacts screen from the Reps menu.
