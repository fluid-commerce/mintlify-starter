...
The included themes call `window.FluidCommerceSDK.updateLocaleSettings()`.
The current SDK bundle exposes the same API as `window.FairShareSDK` for new theme code.
...
document.addEventListener("submit", async (event) => {
  ...
  const sdk = window.FairShareSDK || window.FluidCommerceSDK;
  ...
  try {
    await sdk.updateLocaleSettings({ country, language });
...
Navigating to `window.location.pathname` removes a `lang` or `locale` query override that would otherwise win over the new saved language.
It also lets Fluid render translated content and country-specific pricing for the new selection.
...
Both interfaces call `window.FluidCommerceSDK.updateLocaleSettings()` when the shopper applies the change.

The current SDK bundle maps `window.FluidCommerceSDK` and `window.FairShareSDK` to the same API.
For new theme code, prefer `window.FairShareSDK`.
