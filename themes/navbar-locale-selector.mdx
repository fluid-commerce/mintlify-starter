...
description: "Render country and language choices in a Fluid navbar, apply them with the FairShare SDK, and customize the included Base theme controls."
---

A navbar locale selector has two parts:

- Liquid renders the countries, languages, and current selections.
- Theme JavaScript passes the selected ISO codes to the FairShare SDK and then reloads the storefront path.
...
Both interfaces call the same SDK method when the shopper applies the change.

For new theme code, use `window.FairShareSDK`.
`window.FluidCommerceSDK` remains available only as a backwards-compatible alias for older themes.
