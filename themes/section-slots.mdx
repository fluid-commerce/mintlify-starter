...
<Warning>
  Pre-release: section slots are not yet generally available. This guide applies to environments with the supporting container renderer and editor release enabled. Confirm availability before using these examples in a published theme. Adding a file to your local theme does not install or publish it in a company's theme.
</Warning>
