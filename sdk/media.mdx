...
Hide the CTA panel on one embed by setting `enabled` to `false`. `type` is
still required; `cart` needs no other fields, so it keeps the override short:

```html
<fluid-media-widget
  media-id="unboxing-the-starter-kit"
  cta-options-manual-override='{"type":"cart","enabled":false}'
></fluid-media-widget>
```

This hides the whole panel, including the cart and lead-capture controls. The
same `enabled` key works in `setDefault`, `setForPlaylist`, and `setForMedia`.
